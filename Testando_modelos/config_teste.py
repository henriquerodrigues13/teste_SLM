...
def gerador_de_questoes(modelo: str, instrucao):
    ...
    caminho = Path(__file__).parent / "modelos_local" / nome_pasta

    tokenizer = AutoTokenizer.from_pretrained(str(caminho))
    model = AutoModelForCausalLM.from_pretrained(str(caminho))
    model.eval()
