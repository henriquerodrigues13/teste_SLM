...
def cenario_b(modelo: str, tokenizer, model, metricas):
    ...
    elif dados["ram_pico_mb"] > 2048:
        dados_existentes["eliminado"] = True
        dados_existentes["motivo_eliminacao"] = f"RAM pico {dados['ram_pico_mb']:.1f} MB — limite era 2048 MB"
...
def cenario_b_gguf(llm, modelo, metricas):
    crashes_oom = 0
    json_validos = 0
    json_invalidos = 0
    for i in range(10):
        ...
        mensagens = [
            ...
            **Exemplos de questões existentes para esta habilidade (mantenha consistência de estilo):**
            ...
            Gere 3 questão(ões) novas, inéditas e alinhadas à habilidade {instrucao["habilidadeContext"]["habilidadeId"]}.{sufixo_no_think(modelo)}
            """}
        ]
